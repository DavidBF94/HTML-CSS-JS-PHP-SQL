...
<?php
	...
	/* main_alumnos_paginacion
		...
		-- Descripcion larga --
			...
			En caso de no existir creamos una estructura para crear los botones "Registrarse" e "Iniciar Sesion". También incluye el archivo mod004 y obtiene de la función mod004_obtenerAlumnosPaginacion el array con el código de error, la tabla con la información de los registros y la estructura para el menú de paginación para, a continuación, incluir un archivo vista u otro en función de susodicho código de error. En el propio enlace al controlador fijado en mod004_obtenerAlumnosPaginacion se incluye un número de página que recuperamos con $_GET y fijando un número de registros por página ya tenemos los dos argumentos a enviar a mod004_obtenerAlumnosPaginacion.
		-- Argumentos --
			...
			$arTablaAlumnos						: Es el array respuesta de la función mod004_obtenerAlumnosPaginacion, contiene el código de error, la tabla HTML para representar datos y la estructura para el menú de paginación.
		-- Variables principales -- 
			...
			$datosError							: Es la variable que contiene el código de error de $arTablaAlumnos.
		-- Archivos a los que llama --
			vista_main_alumnos_paginacion / vista_error
		-- Autor --
	...
	*/
	...
	$arTablaAlumnos = mod004_obtenerAlumnosPaginacion( $pag, $numRegistros );
	
	if ( $arTablaAlumnos[ 0 ] !== "002" ) 
	{
		require ( "VISTA/vista_main_alumnos_paginacion.php" );
	} 
	else 
	{
		$datosError = $arTablaAlumnos[ 1 ];
